...
def leftBoard():
    ...
    for box in range(0,11):
        if(box==0):
            boxLabel=Label(gameWindow,font=('Helvetica',30),width=2,height=1,
                            relief='ridge',borderwidth=0,bg='red')
            boxLabel.place(x=xPos,y=screen_height/2-88)
            leftBoxes.append(boxLabel)
            xPos+=50
        else:
            boxLabel=Label(gameWindow,font=('Helvetica',50),width=2,height=1,
                            relief='ridge',borderwidth=0,bg='white')
...
def rightBoard():
    ...
    for box in range(0,11):
        if(box==0):
            boxLabel=Label(gameWindow,font=('Helvetica',30),width=2,height=1,
                            relief='ridge',borderwidth=0,bg='yellow')
            boxLabel.place(x=xPos,y=screen_height/2-88)
...
def gameWindow():
    ...
    rollButton=Button(gameWindow, text='roll dice', fg='black', 
                        font=('Chalkboard SE',15),bg='grey', command=rollDice,
                        width=20, height=5)
